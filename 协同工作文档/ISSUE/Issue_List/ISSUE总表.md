...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ISSUE-0020` | 临时 Cloudflare Worker 反代与安全基线加固 | ops / security hardening / deployment workaround | open / EXTERNAL_BLOCKED | P1 | `ungradeedu.eu.cc` 已进入生产；workers.dev 与 feedback POST 限流门禁已通过；AI bots、源站隔离及业务回归仍待闭环 | Cloudflare / CloudBase 配置执行侧（技术门禁） / 项目总负责人（权限、窗口、账号与证据路由） / 业务方（最终残余风险接受） / ISSUE 管理员（状态维护） | Worker `ungradu-edu-proxy`；生产版本 `d8eff139`；`ungradeedu.eu.cc` 根域与 `www`；产品提交 `60486d5f`；限流生产证据提交 `9aff1117abdcbd8d64aaf9048a1825ba2462208b`；源站隔离开发提交 `6b51f52c`；历史生产复测与门禁记录；`协同工作文档/ISSUE/Open_Issue/ISSUE-0020-临时CloudflareWorker反代与安全基线加固.md` | 保持 Open。新增通过：`feedback-post-rate-limit` Active、1/1，精确命中根域 POST `/api/feedback`，前三次进入应用、第四次 Cloudflare 429 / 1015、11 秒后恢复，基线无回归。剩余：①三类 AI bots 保存为 Block；②双平台 Secret 权限、00:00–01:00 窗口及观察 / 两段灰度 / 403；③专用账号真实已登录 feedback 业务回归；技术门禁完成后再取得届时残余风险接受。唯一下一步：保存三类 AI bots 为 Block 并回传证据 |
