...
- `EMAIL_CODE_SECRET`：服务端验证码哈希密钥。不得暴露到前端，不得写入 Git；生产建议使用独立强随机值。
- `EMAIL_PROVIDER`：邮件发送适配器。生产建议配置为 `smtp`；本地或测试环境可配置为 `console`，但不会向前端返回验证码。
- `SMTP_HOST`：SMTP 服务地址。
- `SMTP_PORT`：SMTP 服务端口，常见 TLS 端口为 `465`。
- `SMTP_SECURE`：是否使用 TLS，默认按 `true` 处理；仅在明确使用明文 SMTP 时设置为 `false`。
- `SMTP_USER`：SMTP 登录账号。
...
- 用户首次通过邮箱验证码注册 / 登录后，可在个人页设置登录密码。
- 设置密码时服务端再次校验：至少 8 位、同时包含字母和数字、两次输入一致。
- 密码使用服务端 `crypto.scrypt` 和唯一 salt 保存，不以明文写入数据库、接口响应、日志或前端状态。
- 密码登录失败统一提示“邮箱或密码不正确”，避免暴露邮箱是否存在。
- 忘记密码 / 重置密码继续通过邮箱验证码确认身份。
...
$env:EMAIL_CODE_SECRET='replace-with-production-email-code-secret'
$env:EMAIL_PROVIDER='smtp'
$env:EMAIL_FROM='replace-with-production-from-email'
$env:SMTP_HOST='replace-with-production-smtp-host'
$env:SMTP_PORT='465'
$env:SMTP_SECURE='true'
$env:SMTP_USER='replace-with-production-smtp-user'
$env:SMTP_PASS='replace-with-production-smtp-password-or-app-token'
...
Remove-Item Env:\EMAIL_CODE_SECRET
Remove-Item Env:\EMAIL_PROVIDER
Remove-Item Env:\EMAIL_FROM
Remove-Item Env:\SMTP_HOST
Remove-Item Env:\SMTP_PORT
Remove-Item Env:\SMTP_SECURE
Remove-Item Env:\SMTP_USER
...
### `/api/auth/email/send-code`、`/api/auth/email/login`、`/api/auth/password/*`、`/api/auth/session`、`/api/auth/logout`

用途：正式邮箱验证码登录 / 注册，并支持设置密码后的邮箱密码登录。新邮箱首次验证码通过后创建账号；老邮箱再次验证码通过后复用原账号；登录态继续使用 `ungradu_auth_session` HttpOnly 签名 Cookie。

- `POST /api/auth/email/send-code`：校验邮箱、频控 60 秒重复发送、生成一次性 6 位验证码、保存验证码哈希和 5 分钟过期时间，并调用服务端邮件发送适配器。
- `POST /api/auth/email/login`：校验邮箱和验证码，拒绝固定测试码 `000000`，拒绝过期、已使用、错误次数过多的验证码；只有成功准备登录会话后才标记验证码已使用并写入服务端签名 Cookie。
- `POST /api/auth/password/set`：已登录用户设置或修改当前邮箱账号密码；服务端按当前签名 Cookie 校验账号归属。
- `POST /api/auth/password/login`：使用邮箱和密码登录，失败时返回通用错误提示，并做基础失败次数限制。
- `POST /api/auth/password/reset`：通过邮箱验证码重置密码；重置成功后才消费验证码。
- `GET /api/auth/session`：从服务端签名 Cookie 读取当前登录态，刷新页面后仍可识别当前用户。
- `POST /api/auth/logout`：清除服务端签名 Cookie。
- CloudBase 集合：`email_login_codes`、`email_login_users`。
- 安全边界：验证码只以哈希形式保存；密码只以 scrypt 哈希和唯一 salt 保存；接口响应不返回验证码、完整邮箱、密码哈希或密码明文；邮件密钥、验证码密钥和会话密钥只从服务端环境变量读取。
- 外部依赖：真实 SMTP 或邮件服务商账号、发件人、Secret、额度、退信告警和试运行邮箱范围由项目总控制人提供。未配置真实邮件发送前，仓库内代码和 fake 测试可以通过，但真实公网用户无法收到验证码邮件。
...
该接口只完成联系方式存档迁移。聊天、联系方式交换请求、需求、家教信息和正式邮箱登录仍待后续 M5 继续迁移，不能视为已经具备完整生产权限模型。
...
- 发布家教信息表单支持多组“学段 + 科目 + 课时费”，可新增或删除多余条目，至少保留 1 组。
- 家教信息列表、详情和“我的家教信息”统一用中文可读格式展示多组收费规则，并兼容旧数据中缺少学段或科目的收费记录。
- 已登录用户访问 `/login` 时不再展示邮箱登录表单，而是展示已登录状态、个人页入口和退出登录入口。
- 顶部导航会根据登录态切换：未登录显示“登录 / 注册”，已登录显示“个人页 / 退出登录”；退出登录前会进行二次确认。
- 主要详情页、发布表单、广场页、登录页、规则页、反馈页和个人页面均提供返回入口。
- 普通用户页面不展示会话来源 ID、测试账号、服务端集合名等内部字段；需要调试的内部字段保留在数据层和代码侧文档中，不直接暴露在普通用户 UI。

生产试运行登录与基础体验修复：

- 登录页提供“验证码登录 / 密码登录 / 忘记密码”三种入口。
- 邮箱验证码登录失败、会话创建失败或登录服务不可用时不会消耗验证码；成功登录后才标记验证码已使用。
- 个人页提供设置登录密码入口；忘记密码通过邮箱验证码重置。
