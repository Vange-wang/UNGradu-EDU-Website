...
function createHandlers() {
  ...
  const runTransaction: EmailAuthAtomicTransactionRunner | undefined =
    typeof database.runTransaction === "function"
      ? (operation) => database.runTransaction((transaction: {
          collection: (name: string) => ReturnType<typeof database.collection>;
        }) => operation({ collection: (name: string) => transaction.collection(name) }))
      : undefined;
...
export async function POST(request: Request) {
  return createHandlers().POST_RESET_PASSWORD(request);
}
